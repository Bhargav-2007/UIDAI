...
export default function AiRiskScoringPage() {
    ...
    const fetchData = async (before?: boolean) => {
        ...
        setError(null);
        try {
            const result = await analyticsApi.getAiRiskScoring(before);
            setData(result);
        } catch (err) {
            const apiError = err as ApiError;
            setError(apiError.message || 'Failed to load AI Risk Scoring data');
            setData(null);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="flex flex-col gap-8 fade-in">
            <div className="flex justify-between items-end border-b border-[#002147]/10 pb-6">
                <div>
                    <h2 className="text-4xl font-serif font-bold text-[#002147] mb-2">AI Risk Scoring</h2>
                    <p className="text-sm font-medium text-slate-500 uppercase tracking-widest">Advanced Risk Assessment & Predictive Security Audit</p>
                </div>
            ...
            <div className="bg-white border border-[#002147]/10 rounded shadow-sm p-8">
                <AnalyticsChart
                    ...
                    title={data?.title || 'AI Risk Scoring'}
                    kpis={data?.kpis || []}
                    isLoading={isLoading}
