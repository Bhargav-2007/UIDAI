...
export default function OperationsModuleNew() {
  const [activeView, setActiveView] = useState('before');
  const [selectedTechnique, setSelectedTechnique] = useState('queue');
  ...
  const techniques = {
    queue: {
      name: 'Queue Theory',
      desc: "Little's Law capacity analysis",
      icon: 'Q',
      fn: queueTheory
    },
    loadbalance: {
      name: 'Load Balancing',
      desc: 'Geographic distribution analysis',
      icon: '=',
      fn: loadBalancing
    },
    throughput: {
      name: 'Throughput Analysis',
      desc: 'Daily processing rate metrics',
      icon: '→',
      fn: throughputAnalysis
    },
    yield: {
      name: 'Yield Analysis',
      desc: 'Update rate and completion metrics',
      icon: '%',
      fn: yieldAnalysis
    },
    pareto: {
      name: 'Pareto Analysis',
      desc: '80/20 rule vital few identification',
      icon: 'P',
      fn: paretoAnalysis
    }
  };
  ...
  return (
    <div className="ml-64 min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100 p-8">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Operational Efficiency</h1>
          <p className="text-slate-400">Real-time monitoring of queue performance, throughput, and yield metrics</p>
        </div>
        ...
        {activeView === 'before' && !loading && result && (
          <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
            <h3 className="text-lg font-semibold mb-4">Raw Time Series Data</h3>
                ...
                <tbody>
                  {rawData.map((val, idx) => (
                    <tr key={idx} className="border-b border-slate-700 hover:bg-slate-700">
                      <td className="p-2 text-slate-400">T{idx + 1}</td>
          ...
          <div className="space-y-6">
            ...
            {result.data.chartData && (
              <ChartRenderer
                ...
                title={`${techniques[result.technique].name} - Performance Metrics`}
              />
            )}

            {/* Insight */}
            <div className="bg-gradient-to-r from-slate-800 to-slate-700 border border-slate-600 rounded-lg p-4">
              <div className="text-xs text-slate-400 uppercase tracking-wider mb-2">Operational Insight</div>
        ...
        {!result && !loading && (
          <div className="text-center py-12 text-slate-400">
            Click any technique above to execute real-time operational analysis
          </div>
        )}
      </div>
    </div>
