...
/**
 * Descriptive Analytics Page Component
 * Displays statistical analysis and time series decomposition
 */
export default function DescriptiveAnalyticsPage() {
    ...
    const fetchData = async (before?: boolean) => {
        ...
        try {
            const result = await analyticsApi.getDescriptiveAnalytics(before);
            setData(result);
        } catch (err) {
            const apiError = err as ApiError;
            setError(apiError.message || 'Failed to load Descriptive Analytics data');
            setData(null);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="flex flex-col gap-8 fade-in">
            <div className="flex justify-between items-end border-b border-[#002147]/10 pb-6">
                <div>
                    <h2 className="text-4xl font-serif font-bold text-[#002147] mb-2">Descriptive Analytics</h2>
                    <p className="text-sm font-medium text-slate-500 uppercase tracking-widest">Statistical Analysis & Time Series Decomposition</p>
                </div>
                <BeforeAfterToggle
                    value={toggleValue}
            ...
            <div className="bg-white border border-[#002147]/10 rounded shadow-sm p-8">
                <AnalyticsChart
                    ...
                    title={data?.title || 'Descriptive Analytics'}
                    kpis={data?.kpis || []}
                    isLoading={isLoading}
