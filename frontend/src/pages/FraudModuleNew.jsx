...
export default function FraudModule() {
  ...
  const [selectedTechnique, setSelectedTechnique] = useState('benford');
  ...
  const techniques = {
    benford: {
      name: "Benford's Law",
      desc: 'Detect manipulation via first-digit distribution',
      icon: '1',
      fn: benfordsLaw
    },
    outliers: {
      name: 'Outlier Detection',
      desc: 'Z-score and IQR-based anomaly detection',
      icon: '!',
      fn: outlierDetection
    },
    patterns: {
      name: 'Pattern Recognition',
      desc: 'Identify recurring time-based patterns',
      icon: '~',
      fn: patternRecognition
    },
    duplicates: {
      name: 'Duplicate Detection',
      desc: 'Identify and flag duplicate enrolments',
      icon: 'x2',
      fn: duplicateDetection
    }
  };
  ...
  const handleExecute = async (techniqueKey) => {
    ...
    try {
      ...
      setRawData(sampleData.slice(0, 100)); // Show first 100 for raw view
  ...
  return (
    <div className="ml-64 min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100 p-8">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Fraud & Integrity Analysis</h1>
          <p className="text-slate-400">Real-time detection of data manipulation and anomalies</p>
        </div>
          ...
          <div className="space-y-6">
            ...
            {result.data.chartData && (
              <ChartRenderer
                ...
                title={`${techniques[result.technique].name} - Visual Analysis`}
              />
            )}

            {/* Insight */}
            <div className="bg-gradient-to-r from-slate-800 to-slate-700 border border-slate-600 rounded-lg p-4">
              <div className="text-xs text-slate-400 uppercase tracking-wider mb-2">Audit Finding</div>
        ...
        {!result && !loading && (
          <div className="text-center py-12 text-slate-400">
            Click any technique above to execute real-time fraud detection
          </div>
        )}
      </div>
    </div>
