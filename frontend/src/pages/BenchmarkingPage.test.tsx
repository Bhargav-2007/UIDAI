import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import { render, screen, waitFor, cleanup } from '@testing-library/react';
import BenchmarkingPage from './BenchmarkingPage';
import { analyticsApi } from '../services/analyticsApi';

vi.mock('../services/analyticsApi', () => ({
    analyticsApi: { getBenchmarking: vi.fn() },
}));
const mockApi = vi.mocked(analyticsApi);

vi.mock('react-chartjs-2', () => ({
    Bar: () => <div data-testid="bar-chart">Bar Chart</div>,
}));

describe('BenchmarkingPage', () => {
    beforeEach(() => { vi.clearAllMocks(); });
    afterEach(() => { vi.restoreAllMocks(); cleanup(); });

    it('should fetch data and render bar chart', async () => {
        mockApi.getBenchmarking.mockResolvedValue({
            labels: ['A'], datasets: [{ label: 'B', data: [1] }],
            kpis: [{ label: 'K1', value: 1 }, { label: 'K2', value: 2 }, { label: 'K3', value: 3 }],
            chartType: 'bar', title: 'T',
        });
        render(<BenchmarkingPage />);
        await waitFor(() => expect(screen.getByText('Benchmarking & Quality')).toBeInTheDocument());
        expect(screen.getByTestId('bar-chart')).toBeInTheDocument();
    });
});
