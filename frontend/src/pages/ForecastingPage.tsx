...
export default function ForecastingPage() {
    ...
    const fetchData = async (before?: boolean) => {
        ...
        setError(null);
        try {
            const result = await analyticsApi.getForecasting(before);
            setData(result);
        } catch (err) {
            const apiError = err as ApiError;
            setError(apiError.message || 'Failed to load Forecasting data');
            setData(null);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="flex flex-col gap-8 fade-in">
            <div className="flex justify-between items-end border-b border-[#002147]/10 pb-6">
                <div>
                    <h2 className="text-4xl font-serif font-bold text-[#002147] mb-2">Forecasting</h2>
                    <p className="text-sm font-medium text-slate-500 uppercase tracking-widest">Predictive Intelligence & Volume Trend Extrapolation</p>
                </div>
